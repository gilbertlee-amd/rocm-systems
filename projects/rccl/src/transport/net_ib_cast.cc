...
static ncclResult_t IbCastStatsCheckFatalCount(struct ncclIbStats* stat, const char* funcName) {
  ...
  if (p == NULL) {
    ...
    *realPort = 0;
    for (int d=0; d<ncclNIbDevs; d++) {
      if (ncclIbMatchVfPath(p, IbCastDevs[d].pciPath)) (*realPort)++;
    }
  }
  *path = p;
  return ncclSuccess;
...
ncclResult_t IbCastInit(void** ctx, uint64_t commId, ncclNetCommConfig_t* config, ncclDebugLogger_t logFunction, ncclProfilerCallback_t profFunction) {
  ...
  if (ncclNIbDevs == -1) {
        ...
        for (int port_num = 1; port_num <= devAttr.phys_port_cnt; port_num++) {
            ...
            for (int dev = devOffset; dev < devCount; ++dev) {
              ...
              if (dev == 0) {
                ...
                NCCLCHECKGOTO(ncclIbGetPciPath(IbCastDevs[ncclNIbDevs].devName, &IbCastDevs[ncclNIbDevs].pciPath, &IbCastDevs[ncclNIbDevs].realPort), ret, fail);
              } else {
              ...
              INFO(NCCL_NET, "NET/IB: [%d] %s:%s:%d/%s provider=%s speed=%d context=%p pciPath=%s ar=%d", d, devices[d]->name, devices[d]->dev_name,
                   ...
                   IbCastDevs[ncclNIbDevs].pciPath, IbCastDevs[ncclNIbDevs].ar);
    ...
    ncclIbUseInline = ncclParamIbCastUseInline();
...
RCCL_PARAM(CastForceEnableGdrdma, "FORCE_ENABLE_GDRDMA", -1);
...
static void ibGdrSupportInitOnce() {
  ...
  if (rcclParamCastForceEnableGdrdma() == 1) {
...
const char* IbCastReqTypeStr[] = { "Unused", "Send", "Recv", "Flush" };
...
ib_connect:
  ...
  for (int q = 0; q < comm->base.nqps; q++) {
    ...
    if (remQpInfo->ece_supported) {
      ...
      struct ncclIbDev* ibDev = IbCastDevs + ibDevN;
    ...
    NCCLCHECKGOTO(IbCastRtrQp(qp, &commDev->base.gidInfo, remQpInfo->qpn, remDevInfo, false, remMeta.tc, remMeta.sl), ret, fail);
    NCCLCHECKGOTO(IbCastRtsQp(qp), ret, fail);
  }
...
ib_recv:
  ...
  for (int q = 0; q < rComm->base.nqps; q++) {
    ...
    NCCLCHECKGOTO(IbCastRtrQp(qp->qp, &rCommDev->base.gidInfo, remMeta.qpInfo[q].qpn, remDevInfo, true, remMeta.tc, remMeta.sl), ret, fail);
    NCCLCHECKGOTO(IbCastRtsQp(qp->qp), ret, fail);
  ...
  for (int i = 0; i < rComm->base.vProps.ndevs; i++) {
    ...
    if (rComm->flushEnabled) {
      ...
      NCCLCHECKGOTO(IbCastCreateQp(ibDev->portNum, &rCommDev->base, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE, &rComm->base.stats, &rCommDev->gpuFlush.qp, channel_id, true, NCCL_CTS_QP_SLOT_INVALID), ret, fail);
